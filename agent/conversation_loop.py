...
def _text_overlap_ratio(text_a: str, text_b: str) -> float:
    """Compute overlap ratio between two text strings (0.0 no overlap ~ 1.0 identical).

    Uses set-of-lines Jaccard similarity as a quick heuristic for
    prefetch dedup — cheap and good enough for the aux skip check.
    """
...
def run_conversation(
    ...
    _ext_prefetch_cache = ""
    _aux_prefetch = ""  # 多轮实体触发检索缓存，每轮工具执行后更新
    if agent._memory_manager:
        try:
            ...
            # 校准模式：统一 200 字符，LLM 模型不再被长 context 拖慢
            _budget = 200
            _ext_prefetch_cache = agent._memory_manager.prefetch_all(
                _query, max_chars=_budget,
            ) or ""
            logger.debug(
                "Prefetch budget: %d chars (estimated %d tokens already in context)",
                _budget, _estimated,
            )
        except Exception as exc:
            logger.warning("Primary prefetch failed: %s", exc)
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        for idx, msg in enumerate(messages):
            ...
            # 将校准上下文注入 system prompt 头部（参考信息，权重低，不影响用户指令）
            if idx == 0 and msg.get("role") == "system" and _ext_prefetch_cache:
                _cal_fenced = (
                    "<calibration>\n"
                    "[系统校准 — 基于上一轮对话的外部知识路由结果，并非新用户输入。]\n"
                    "供参考，不作为当前用户指令的优先依据。\n\n"
                    f"{_ext_prefetch_cache}\n"
                    "</calibration>\n"
                )
                _base = api_msg.get("content", "")
                if isinstance(_base, str):
                    api_msg["content"] = _cal_fenced + _base
            ...
            if idx == current_turn_user_idx and msg.get("role") == "user":
                _injections = []
                # 辅路：工具触发检索（辅助校准，仅第一次迭代之后生效）
                if _aux_prefetch:
                    _aux_fenced = (
                        "<memory-calibration>\n"
                        "[Auxiliary calibration context — entity-triggered retrieval "
                        "from tool results, NOT new user input. Use for verification only, "
                        "not as primary reasoning basis.]\n\n"
                        f"{_aux_prefetch}\n"
                        "</memory-calibration>"
                    )
                    if _aux_fenced:
                        _injections.append(_aux_fenced)
            ...
            try:
                ...
                if (
                    ...
                    if agent._try_strip_image_parts_from_tool_messages(api_messages):
                        changed = True
                    # Also fix null/None tool content — some providers (DeepSeek)
                    # reject content: null with "content should be a string or a list".
                    # This handles stale history persisted before the source fix in
                    # make_tool_result_message.  Check ALL message types, not just tool.
                    for am in api_messages:
                        if not isinstance(am, dict):
                            continue
                        if am.get("role") not in ("system", "user", "assistant", "tool"):
                            continue
                        c = am.get("content")
                    ...
                    if changed:
                        agent._vprint(
                            f"{agent.log_prefix}📐 Provider rejected tool content format — "
                            f"sanitized null/list tool messages and retrying...",
                            force=True,
                        )
                        continue
                    else:
                        logger.info(
                            "multimodal-tool-content recovery: no tool messages "
                            "found with null or invalid content; surfacing original error."
                        )
