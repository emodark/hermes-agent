...
def get_high_confidence_instincts(threshold: float = 0.7, max_instincts: int = 10,
    ...
    Args:
        ...
        max_instincts: Max number of instincts to return.
        domain_filter: If set, only return instincts with this domain (e.g. "behavior").
    """
    instincts = _load_instincts()
    if domain_filter:
        instincts = [i for i in instincts if i.get("domain") == domain_filter]
...
def inject_instincts_prompt(threshold: float = 0.7, max_chars: int = 400,
    ...
    Args:
        ...
        domain_filter: If set, only include instincts with this domain (e.g. "behavior").

    Returns empty string if no high-confidence instincts exist.
    Uses 15-minute in-memory cache keyed by (max_chars, domain_filter) to avoid repeated YAML IO.
    """
    global _INSTINCT_PROMPT_CACHE, _INSTINCT_PROMPT_CACHE_TS
    cache_key = f"mc={max_chars},df={domain_filter or 'none'}"
    now = time.monotonic()
    ...
    instincts = get_high_confidence_instincts(threshold, max_instincts=20, domain_filter=domain_filter)
    if not instincts:
    ...
    for instinct in instincts:
        ...
        prefix = "⚠️ " if domain == "behavior" and conf < 0.9 else ""
        line = f"- [{domain}] {prefix}{desc} (confidence: {conf:.0%})"
...
def track_kr_compliance() -> Dict[str, Any]:
    """分析观察数据，追踪知识路由先行习惯的合规率。

    按 session 分组统计，检查分析型 session 中 knowledge_router
    是否在前 5 步内被调用。更新 instincts.yaml 中对应条目的置信度。

    Returns:
        Dict with {sessions_total, sessions_analysis, kr_first, kr_first_rate}
    """
    from collections import defaultdict

    obs = _load_observations()
    if not obs:
        return {"sessions_total": 0, "sessions_analysis": 0,
                "kr_first": 0, "kr_first_rate": 0.0}

    sessions = defaultdict(list)
    for o in obs:
        sid = o.get("session_id", "unknown")
        sessions[sid].append(o)

    ANALYSIS_TOOLS = {"terminal", "execute_code", "web_search",
    ...
    for sid, calls in sessions.items():
        if len(calls) < 3:
            continue
        sorted_calls = sorted(calls, key=lambda x: x.get("timestamp", ""))
        tool_names = [c.get("tool", "") for c in sorted_calls]
        tool_args = [str(c.get("args_preview", {})) for c in sorted_calls[:5]]

        has_analysis = any(t in ANALYSIS_TOOLS for t in tool_names[:10])
        if not has_analysis:
            continue
        analysis_sessions += 1

        # KR 可能通过 terminal("python3 knowledge_router.py...") 调用
        combined_args = " ".join(tool_args).lower()
        if "knowledge_router" in combined_args:
            kr_first_sessions += 1

    rate = kr_first_sessions / max(analysis_sessions, 1)

    # 更新 instincts.yaml（直接写，绕过 save_instincts 的 merge 保护）
    try:
        yaml_path = _HERMES_HOME / "instincts" / "instincts.yaml"
        instincts = _load_instincts()
        for inst in instincts:
            if inst.get("name") == "knowledge_router_first":
                new_conf = round(min(max(0.7, 0.7 + (rate - 0.5) * 0.3), 0.9), 2)
                inst["confidence"] = new_conf
                inst["evidence"] = {
                    "source": "compliance_tracking",
                    "compliance_rate": rate,
                    "sessions_analyzed": analysis_sessions,
                    "last_updated": datetime.now(timezone.utc).isoformat()
                }
                # 直接写 YAML 文件（保持格式兼容）
                lines = [
                    "# Hermes Agent Instincts — auto-generated, do not edit manually",
                    f"# Generated: {datetime.now(timezone.utc).isoformat()}",
                    f"# Total: {len(instincts)} instincts",
                    "",
                    "instincts:",
                ]
                for inst_i in sorted(instincts, key=lambda x: x.get("confidence", 0), reverse=True):
                    lines.append(f"  - name: {inst_i.get('name', 'unknown')}")
                    lines.append(f"    description: \"{inst_i.get('description', '')}\"")
                    lines.append(f"    domain: {inst_i.get('domain', 'general')}")
                    lines.append(f"    confidence: {inst_i.get('confidence', 0):.2f}")
                    lines.append(f"    scope: {inst_i.get('scope', 'session')}")
                    ev = inst_i.get("evidence", {})
                    lines.append(f"    evidence: {json.dumps(ev, ensure_ascii=False)}")
                with open(yaml_path, "w", encoding="utf-8") as f:
                    f.write("\n".join(lines) + "\n")
                break
    except Exception as e:
        logger.warning("track_kr_compliance: %s", e)

    return {
        "sessions_total": len(sessions),
        "sessions_analysis": analysis_sessions,
        "kr_first": kr_first_sessions,
        "kr_first_rate": round(rate, 3),
    }


def _write_instincts_yaml(instincts: List[Dict]) -> None:
    """将 instincts 列表写回 YAML 文件（通用 helper）。"""
    lines = [
        "# Hermes Agent Instincts — auto-generated, do not edit manually",
        f"# Generated: {datetime.now(timezone.utc).isoformat()}",
        f"# Total: {len(instincts)} instincts",
    ...
    for inst in sorted(instincts, key=lambda x: x.get("confidence", 0), reverse=True):
        lines.append(f"  - name: {inst.get('name', 'unknown')}")
        lines.append(f"    description: \"{inst.get('description', '')}\"")
        lines.append(f"    domain: {inst.get('domain', 'general')}")
        lines.append(f"    confidence: {inst.get('confidence', 0):.2f}")
        lines.append(f"    scope: {inst.get('scope', 'session')}")
...
def apply_session_end_decay(session_id: str) -> None:
    """Session结束时衰减知识路由先行置信度。

    检查给定 session 的观测记录，如果是分析型 session 且没有调用
    知识路由，则将 knowledge_router_first 的 confidence -= 0.02。

    安全地调用：非分析型 session、短 session、已有 KR 调用都不衰减。
    """
    if not session_id:
        return

    obs = _load_observations()
    session_obs = [o for o in obs if o.get("session_id") == session_id]

    if len(session_obs) < 3:
        return  # 太短的 session 不统计

    # 按时间排序
    sorted_obs = sorted(session_obs, key=lambda x: x.get("timestamp", ""))
    tool_names = [o.get("tool", "") for o in sorted_obs[:10]]

    # 判断是否分析型 session
    ANALYSIS_TOOLS = {"terminal", "execute_code", "web_search", "web_extract",
                      "browser_navigate", "hindsight_recall", "hindsight_reflect"}

    has_analysis = any(t in ANALYSIS_TOOLS for t in tool_names)
    if not has_analysis:
        return  # 非分析型 session，不衰减

    # 检查是否调用了知识路由
    combined_args = " ".join(
        [str(o.get("args_preview", {})) for o in sorted_obs[:10]]
    ).lower()
    if "knowledge_router" in combined_args:
        return  # 调用了，不衰减
    ...
    for inst in instincts:
        if inst.get("name") == "knowledge_router_first":
            ...
            new_conf = round(max(0.7, old_conf - 0.02), 2)
            inst["confidence"] = new_conf
            inst["evidence"] = {
                "source": "session_end_decay",
                "previous_confidence": old_conf,
                "session_id": session_id,
                "reason": "analysis_session_without_knowledge_router",
                "last_updated": datetime.now(timezone.utc).isoformat(),
            }
            found = True
            logger.info(
                "Session decay: knowledge_router_first %.2f → %.2f (session %s)",
                old_conf, new_conf, session_id,
            )
            break

    if found:
        _write_instincts_yaml(instincts)
