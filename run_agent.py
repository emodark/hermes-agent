...
class AIAgent:
    ...
    def run_conversation(
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            reasoning_content_retry_attempted = False
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    # ── DeepSeek thinking mode: reasoning_content recovery ──
                    # DeepSeek V4 (and some other providers) require the
                    # `reasoning_content` field from previous assistant
                    # responses to be passed back on every turn.  If any
                    # assistant message lacks it, the API rejects the request
                    # with HTTP 400.  Recovery: ensure every assistant message
                    # in `messages` has `reasoning_content` (defaulting to
                    # empty string), then rebuild API messages.
                    def _patch_reasoning_content(msg_list: list):
                        """Ensure all assistant messages in *msg_list* have
                        ``reasoning_content`` set.  Tries the normalized
                        ``reasoning`` field first; falls back to ``""``.
                        Returns a &#96;patched, total&#96; count string for logging."""
                        _patched = 0
                        for _m in msg_list:
                            if isinstance(_m, dict) and _m.get("role") == "assistant":
                                if "reasoning_content" not in _m or _m["reasoning_content"] is None:
                                    _fallback = _m.get("reasoning")
                                    if isinstance(_fallback, str) and _fallback:
                                        _m["reasoning_content"] = _fallback
                                    else:
                                        _m["reasoning_content"] = ""
                                    _patched += 1
                        return len(msg_list), _patched

                    if (
                        classified.reason == FailoverReason.reasoning_content_required
                        and not reasoning_content_retry_attempted
                    ):
                        reasoning_content_retry_attempted = True
                        _total, _patched = _patch_reasoning_content(messages)
                        # api_messages was built before the retry loop (line ~9134)
                        # and is never rebuilt on ``continue``.  The next retry
                        # uses api_messages → api_kwargs, so we must patch both.
                        _api_total, _api_patched = _patch_reasoning_content(api_messages)
                        self._vprint(
                            f"{self.log_prefix}⚠️  Reasoning content required for thinking mode — "
                            f"patched reasoning_content on {_patched}/{_total} messages "
                            f"(api_messages: {_api_patched}/{_api_total}), retrying...",
                            force=True,
                        )
                        logging.warning(
                            "%sReasoning content recovery: patched %d/%d messages "
                            "(api_messages: %d/%d)",
                            self.log_prefix, _patched, _total, _api_patched, _api_total,
                        )
                        continue
