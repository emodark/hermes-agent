...
def dream() -> dict:
    ...
    # ── Step 8: 行为合规追踪（知识路由习惯） ──
    try:
        from agent.instincts import track_kr_compliance
        compliance = track_kr_compliance()
        report["compliance"] = compliance
        if compliance["sessions_analysis"] > 0:
            report["steps"].append(
                f"compliance: {compliance['kr_first_rate']:.1%} kr-first "
                f"({compliance['kr_first']}/{compliance['sessions_analysis']})"
            )
        else:
            report["steps"].append("compliance: no sessions (skip)")
    except Exception as e:
        report["steps"].append(f"compliance: FAILED ({e})")
        import traceback
        report["steps"].append(traceback.format_exc()[-150:].replace(chr(10), " | "))
...
def format_report(report: dict) -> str:
    ...
    if comp and comp.get("sessions_analysis", 0) > 0:
        lines.append(f"  🎯 习惯: 知识路由先行 {comp['kr_first_rate']:.0%} "
                     f"({comp['kr_first']}/{comp['sessions_analysis']} 分析session)")
