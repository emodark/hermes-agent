...
class TestFailoverReason:
    ...
    def test_enum_members_exist(self):
        expected = {
            ...
            "thinking_signature", "long_context_tier", "reasoning_content_required", "unknown",
        }
...
class TestClassifyApiError:
    ...
    # ── Provider-specific: DeepSeek reasoning_content required ──

    def test_reasoning_content_required(self):
        """400 from DeepSeek V4 thinking mode — reasoning_content must be passed back."""
        e = MockAPIError(
            "The `reasoning_content` in the thinking mode must be passed back to the API.",
            status_code=400,
        )
        result = classify_api_error(e, provider="deepseek", model="deepseek-v4-flash")
        assert result.reason == FailoverReason.reasoning_content_required
        assert result.retryable is True
        assert result.should_fallback is False

    def test_reasoning_content_required_non_deepseek(self):
        """400 with 'reasoning_content' in message but without 'must be passed back' → not classified as reasoning_content_required."""
        e = MockAPIError(
            "reasoning_content field is not supported with this model",
            status_code=400,
        )
